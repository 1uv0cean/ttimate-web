...
export const ShareModal = ({
  ...
  const shareUrl = typeof window !== 'undefined' 
    ? `${window.location.origin}/result?child=${childYear}&parent=${parentYear}`
    : '';
  ...
  const handleKakaoShare = () => {
    ...
    const shareText = `${childYear}년생 ${getZodiacEmoji(childAnimal)} 자녀와 ${parentYear}년생 ${getZodiacEmoji(parentAnimal)} 부모의 띠 궁합은 ${compatibilityGrade}!\n\n띠메이트에서 확인해보세요: ${shareUrl}`;
    ...
    try {
      window.Kakao.Share.sendDefault({
        ...
        content: {
          ...
          imageUrl: `${window.location.origin}/api/og?child=${childYear}&parent=${parentYear}`,
          link: {
    ...
    >
        <div className="space-y-3">
          ...
          <div className="rounded-lg bg-gray-50 p-3">
            <Typography variant="small" className="break-all text-gray-600 text-xs">
              {shareUrl}
            </Typography>
          </div>
        </div>
